...
const long intervaloA = 300;
const long intervaloB = 300;
const long intervaloC = 300;
...
void loop() {
  ...
  if (client.connected()){ //Detecta se há clientes conectados no servidor.
    if (client.available() > 0){ //Verifica se o cliente conectado tem dados para serem lidos.
      ...
      if ((client.remoteIP() == IPA) && (pacoteEsperado == tamanho)) {
      ...
      if ((client.remoteIP() == IPB) && (pacoteEsperado == tamanho)) {
      ...
      if ((client.remoteIP() == IPC) && (pacoteEsperado == tamanho)) {
